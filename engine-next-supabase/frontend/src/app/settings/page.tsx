...
const DEFAULT_SETTINGS: Setting[] = [
  { key: 'AI_MODEL', value: 'llama-3.3-70b-versatile', description: 'Primary cognitive model for classification.' },
  { key: 'DEDUPE_THRESHOLD', value: '0.8', description: 'Sensitivity for fuzzy duplicate detection (0.0-1.0).' },
  { key: 'LATER_DELAY_MIN', value: '30', description: 'Default deferral period in minutes.' },
  { key: 'FATIGUE_LIMIT', value: '5', description: 'Max high-priority alerts / hour / user.' }
];
  ...
  const fetchSettings = async () => {
    try {
      ...
      const merged = DEFAULT_SETTINGS.map(def => {
        ...
        return found ? { key: found.key, value: found.value, description: found.description } : def;
      });
      setSettings(merged);
    } catch (e) {
  ...
  if (loading) {
    return (
      <div className="py-20 flex flex-col items-center gap-4">
        ...
        <p className="text-zinc-500 font-black uppercase tracking-widest text-[10px]">Synchronizing Engine State...</p>
      </div>
    );
  }
    ...
    <div className="space-y-10">
        ...
        >
          <div className="space-y-8 p-3">
            <SettingInput 
              ...
              description={settings.find(s => s.key === 'AI_MODEL')?.description || ''}
            ...
            >
              <select 
                 value={settings.find(s => s.key === 'AI_MODEL')?.value}
                 onChange={(e) => { updateVal('AI_MODEL', e.target.value); handleSave('AI_MODEL'); }}
              ...
              >
                <option value="llama-3.3-70b-versatile">Llama 3.3 70B (Fast/Optimal)</option>
                <option value="gemini-flash-latest">Gemini 2.0 Flash (Fast Reasoning)</option>
                <option value="gemini-pro-1.5">Gemini 1.5 Pro (Deep Context)</option>
            ...
            <SettingInput 
              ...
              description={settings.find(s => s.key === 'DEDUPE_THRESHOLD')?.description || ''}
              value={settings.find(s => s.key === 'DEDUPE_THRESHOLD')?.value || ''}
        ...
        >
          <div className="space-y-8 p-3">
            <SettingInput 
              ...
              description={settings.find(s => s.key === 'FATIGUE_LIMIT')?.description || ''}
              value={settings.find(s => s.key === 'FATIGUE_LIMIT')?.value || ''}
            ...
            <SettingInput 
              ...
              description={settings.find(s => s.key === 'LATER_DELAY_MIN')?.description || ''}
              value={settings.find(s => s.key === 'LATER_DELAY_MIN')?.value || ''}
      ...
      <div className="glass-card p-6 border-l-4 border-l-purple-600 flex items-center justify-between">
         <div className="flex gap-4 items-center">
            ...
            <div>
               <p className="text-sm font-black text-white uppercase tracking-tighter">Persistence Mode</p>
...
function SettingsCard({ icon: Icon, title, subtitle, children }: { icon: any; title: string; subtitle: string; children: React.ReactNode }) {
  return (
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        <div className="flex gap-3">
          ...
          <div>
            <label className="text-xs font-black text-zinc-300 uppercase tracking-widest">{label}</label>
            <p className="text-[10px] text-zinc-600 font-bold mt-0.5">{description}</p>
          </div>
        </div>
      </div>
